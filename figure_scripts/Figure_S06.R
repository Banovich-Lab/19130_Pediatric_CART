...
tmp_CTs <- as.character(levels(as.factor(product_obj_T@meta.data$ct_T_states)))
tmp_color <- setNames(names(t_cell_colors), t_cell_colors) # swap names and values so I can subset
tmp_color_sub <- tmp_color[tmp_color %in% c(tmp_CTs)] # subset ct_color to only keep colors in Product UMAP
...
product_obj_T_cp <- product_obj_T
DefaultAssay(product_obj_T_cp) <- "SoupX_RNA" 
product_obj_T_cp <- NormalizeData(product_obj_T_cp)
VariableFeatures(product_obj_T_cp) <- rownames(product_obj_T_cp)
product_obj_T_cp <- ScaleData(product_obj_T_cp)

p <- DotPlot(product_obj_T_cp, features = all_markers, 
...
ct_colors_sub_heatmap <- c(list(Product = ct_colors_sub))
...
pdf("/home/aoill/plots/Figure_S06.pdf",
    width = 7, height = 7)
