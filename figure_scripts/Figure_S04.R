...
# Description: Supplemental figure - Dotplot heatmaps for cell state annotation 
# for CSF T cells
#==============================================================================#
...
tmp_CTs <- as.character(levels(as.factor(csf_obj_T@meta.data$ct_T_states)))
tmp_color <- setNames(names(t_cell_colors), t_cell_colors) # swap names and values so I can subset
tmp_color_sub <- tmp_color[tmp_color %in% c(tmp_CTs)] # subset ct_color to only keep colors in CSF UMAP
...
p <- DotPlot(csf_obj_cp, features = all_markers, 
...
ct_colors_sub_heatmap <- c(list(CSF = ct_colors_sub))
...
pdf("/home/aoill/plots/Figure_S04.pdf",
    width = 7, height = 7)
